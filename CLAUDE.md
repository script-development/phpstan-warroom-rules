...
| `ForbidCredentialCastBypassRule` | War-room §Explicit over implicit (#1) + §Rotation-invariant credential handling (#10) | `forbidCredentialCastBypass.castBypassedByBuilderWrite` / `.modelSourceUnreadable` / `.castMapIncomplete` / `.configuredModelMissing` (flags a `hashed` / `encrypted` / `encrypted:*` or encrypting class cast (`AsEncryptedArrayObject` / `AsEncryptedCollection`, `::class` or string form, pinned against the casts `illuminate/database` ships) column appearing as a key in a QUERY-BUILDER write payload — `update`/`insert`/`insertOrIgnore`/`insertGetId`/`upsert`/`updateOrInsert` plus the increment family (`increment`/`decrement`/`incrementEach`/`decrementEach`, whose EXTRA payload reaches SQL via `update(array_merge($columns, $extra))`) on an Eloquent `Builder`, query `Builder` or `Relation`. Casts fire on the model path only, so a builder write stores the credential in plaintext with a green suite. The model path and the model-routing builder verbs (`create`/`updateOrCreate`/`firstOrCreate`/`createOrFirst`) are silent STRUCTURALLY — the receiver type gate, not an exemption list — with ONE exception the gate deliberately admits: the eight increment/decrement verbs (`MODEL_BYPASSING_METHODS`) are read on a `Model` receiver too, because `Model::__call()` forwards them and `Model::incrementOrDecrement()` hands the EXTRA payload to the query builder uncast. Model comes from the builder/relation generic, read PER UNION BRANCH; `DB::table('…')` carries none and resolves only via the opt-in `credentialCastTableModels` map, default `[]`. Cast maps read from model SOURCE via the injected `@currentPhpVersionRichParser` — both `casts()` and `$casts` — then resolved as PHP resolves them, NOT merged across the ancestry: `$casts` is a property so ONE declaration survives (most derived, replacing), `casts()` is a SINGLE virtual dispatch so only the nearest body runs and an ancestor contributes only through an explicit `parent::casts()` (carried by `array_merge(parent::casts(), …)`, a spread, or a bare pass-through). Laravel's `array_merge($this->casts, $this->casts())` makes the METHOD half win on a shared column regardless of source order. ⚠ The merge-everything reading was wrong on 9 of the 23 shapes `CastDispatchShapes.php` first held, 8 of them FALSE POSITIVES inventing a cast the model does not have; a first-match trait walk is wrong on 2 others (`insteadof`, discarded `parent::casts()`). The shape test computes its expectation from PHP itself. Fail-open shapes are each reported under their own identifier rather than reading as castless: `.modelSourceUnreadable` (source cannot be located/parsed), `.castMapIncomplete` (read, but a declaration carries no array literal at all — `return self::CASTS;` — OR a `casts()` body carries no `return` whatsoever, which no valid PHP can, so the source handed to the rule is not the source that runs), `.configuredModelMissing` (`credentialCastTableModels` names a nonexistent class). ⚠ **The parser choice is load-bearing, not incidental (WR-1462).** This is the only rule that parses a file the analyser is not analysing, and `@defaultAnalysisParser` is `CachedParser(PathRoutingParser)`, which routes a file outside the analysed set through `CleaningParser` and empties every method body. Wired that way the rule read every non-analysed model as castless and reported NOTHING — measured inert on three consumer territories, ublgenie under a full `composer phpstan` run over four injected violations, because a warm result cache narrows the analysed set to the changed files. The package's own suite could not see it: `PHPStan\Testing\TestCase.neon` overrides `currentPhpVersionSimpleParser!` to `@currentPhpVersionRichParser`, so a `RuleTestCase` never gets a stripped AST. Reproduction needs an injected parser that routes one file through the real `CleaningParser`; a container-resolution test now asserts the wired parser's class. Payload keys read from the CONSTANT ARRAY TYPE, so a hoisted variable is caught. Seed lokalekeuze PR #65. shipped in v0.9.0 INERT; the parser fix shipped in v0.10.0 — a consumer still on `^0.9` runs the inert wiring until its `^0.10` pin bump) |
| `ForbidAdHocDateParsingRule` | ADR-0020 Amd 1 (Semantic Boundary Types) + ADR-0031 | `forbidAdHocDateParsing.stringParsedOutsideBoundary` (type-aware; flags date/time construction from a STRING outside the configured boundary namespaces — a static call on a `DateTimeInterface` subtype (covers the whole Carbon family, since `CarbonInterface` extends it) or the `Illuminate\Support\Facades\Date` facade with a method in the 13-entry parse/create set, `new` on a `DateTimeInterface` subtype, and the `strtotime` / `date_create` / `date_create_immutable` / `date_create_from_format` / `date_create_immutable_from_format` / `date_parse` / `date_parse_from_format` functions — each only when the argument in the verb's DECODED SLOT is present and not provably non-string (`mixed` and `int|string` fire; integer components, `null`, an existing value object and a zero-argument clock read do not). The slot is per verb rather than argument zero (`createFromFormat` decodes its second parameter, `create` its `$year`) and is read by argument NAME first, position second, so `create(timezone: …)` is silent and `create(month: 1, year: $raw)` fires; the spellings are pinned against the real Carbon and native signatures by a test. Allowed prefixes come from `dateParsingNamespaces`, default `App\Support\Time` / `App\Support\DateTime` / `App\Casts` — the two boundary doors of ADR-0020 Amd 1 — matched on a namespace SEPARATOR, so `App\Casts\Money` is inside and `App\CastsReport` is not. The static method name is folded (`Carbon::PARSE()` fires) and the function callee is RESOLVED through the `ReflectionProvider`, so a same-namespace `strtotime()` helper is silent and `use function strtotime as decode;` fires. Class resolution is by TYPE: an aliased `use Carbon\CarbonImmutable as C;` fires, a local class merely NAMED `Carbon` does not. Deliberate misses, each because no string is decoded: any listed call whose decoded slot is absent or provably not a string, the `now`/`today`/`yesterday`/`tomorrow` clock reads, the `createFromTimestamp*` family, `instance`, `fromSerialized`, instance calls on an already-decoded value, and a dynamic class expression. No per-call-site comment exemption — a site that needs one is an unnamed boundary namespace. Seed war-room enforcement queue #222; 30 of 354 crit findings in one week were date/cursor-bound semantics. shipped in v0.9.0) |
| `ForbidCredentialDerivedCacheKeyRule` | War-room §Rotation-invariant cache keys (#10) | `forbidCredentialDerivedCacheKey.keyFromEncryptedAttribute` (the only rule built on a COLLECTOR: `CacheKeyTaintCollector` records per-node flow facts and the rule solves them over the whole analysed set, so a digest built in a key object one class away from the `get()` is caught. Sources are reads on a receiver whose TYPE is a model with an encrypting cast — the cast map is `ForbidCredentialCastBypassRule`'s, injected by type and resolved at RULE time so a warm result cache cannot hold a stale verdict. Sinks: key arguments on the `Illuminate\Contracts\Cache` / `Illuminate\Cache` families, the `Cache` facade, `cache()`, and `RateLimiter`. Calls into analysed code resolve through per-call-site return summaries. Limits are listed in the rule docblock. Seed war-room enforcement queue #24 / laravel-skeleton PR #48. Unreleased) |
...
> Last synced: 2026-08-11 (ADR projections). Release-status markers in this section updated by the war room 2026-09-21 for v0.10.0.
