...
Detection is keyed on the SHAPE, never on a helper-name list: the callee must be declared under a configured first-party namespace, take at least one `mixed` (or untyped) parameter, and return a nullable scalar. The namespace prefixes are configurable, default `App`:
