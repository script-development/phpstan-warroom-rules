...
|---|---|---|---|
| `EnforceActionResultDtoRule` | `enforceActionResultDto.arrayReturnFromExecute` | The `execute()` method of `App\Actions\*` classes (namespace prefix) | Declaring an `array` native return type (bare `array`, `?array`, a union/intersection member `array|SomeResultDto`, or `iterable` in any of those forms) on `execute()` is an error — a compound result is a struct that should be a Result DTO. Everything else passes: Result-DTO classes, `void`, models, `Collection`, scalars, `bool`, or no declared type. **Signature-only** — a phpdoc-only `@return array{...}` on an untyped `execute()` is a deliberate miss (every consumer enforces native return types via its own tooling). **No `list<T>` carve-out** (`list<string>` is spelled `array` natively and converts all the same). Only the `execute()` method is inspected; private array-returning helpers are silent. Doctrine: ADR-0020 (Input/Result DTO Split by Usage Direction) + ADR-0011 (Action Class Architecture). Seed: kendo PR #1653. |
...
| `ForbidResourceWrappedInJsonResponseRule` | `forbidResourceWrappedInJsonResponse.resourceWrapped` | `response()->json(...)` (the helper `FuncCall` receiver, matched by AST shape) and `new Illuminate\Http\JsonResponse(...)` in any code whose NAMESPACE starts with a `controllerNamespacePrefixes` entry (default `App\Http\Controllers`) — the scope is the namespace, not the enclosing class, so a namespaced helper function there is in scope too — when the FIRST argument's resolved type is a subtype of `Illuminate\Http\Resources\Json\JsonResource` | Wrapping a `JsonResource` in an explicit JSON response is an error — a resource is already `Responsable`, so the wrap double-wraps the payload and discards the resource's own response shaping; return the resource directly (`return XxxResource::fromModel($model);`). **Type-aware, deliberately:** a plain array / DTO / scalar / message envelope and `response()->json(null, 204)` are silent, and a resource nested under a named key (`response()->json(['registrations' => Resource::collect(...)])`) is a deliberate envelope whose first argument is an `array`, so it stays silent by design. Doctrine: war-room §Explicit over implicit (#1) + ADR-0009 (resources own their own serialization). Shares `controllerNamespacePrefixes` with `ForbidEloquentMutationInControllersRule` and `EnforceCurrentUserAttributeRule`. |
...
```neon
parameters:
    credentialCastTableModels:
        users: 'App\Models\User'
        api_keys: 'App\Models\ApiKey'
```

Only tables whose model declares a credential cast (`hashed`, `encrypted`,
`encrypted:*`, or an encrypting class cast) are worth listing; a mapped table whose model has no credential cast changes
...
Two halves, two different PHP rules, and the difference decides whether a write
is flagged:

| Shape | Effective at runtime | Rule |
|---|---|---|
| `casts()` on the model, or inherited with no override | the declared map | flagged |
| `casts()` override that does **not** call `parent::casts()` | ONLY the override's map — the ancestor's body never runs | not flagged |
| `casts()` override calling `parent::casts()` (directly, via `array_merge`, or via a spread) | both, nearer wins | flagged |
| `$casts` property, own or inherited | the declared map | flagged |
| `$casts` property redeclared in a child | ONLY the child's — a property redeclaration replaces | not flagged |
| both `$casts` and `casts()` on one class | the METHOD wins, whatever the source order | per the method |
| trait `casts()` with a class-declared `casts()` too | ONLY the class's — the trait's body never runs | not flagged |
| trait `casts()` or `$casts` with no class declaration | the trait's map | flagged |
...
Several different things can stop the rule from reading a complete cast map, and
...
| `forbidCredentialCastBypass.castMapIncomplete` | the `casts()` body carries no `return` at all, which no valid PHP can — the method declares an `array` return type, so the source the rule was handed is not the source that runs | fix the analysis wiring, not the model — see the note below |
...
All are deliberately independent of the payload: with an incomplete map, a
...
**This rule reads model source, so it needs a parser that keeps method bodies.**
Unlike every other rule in the package it parses a file it is not being asked to
analyse — the model named by the write's receiver type. PHPStan's
`@defaultAnalysisParser` routes any file outside the current invocation's
analysed set through `CleaningParser`, which empties every method body, so a
`casts()` read that way comes back with no statements and the map resolves EMPTY.
`extension.neon` therefore wires `@currentPhpVersionRichParser`, and a
container-resolution test asserts the wired parser's class. In `v0.9.0` the
rule was wired to `@defaultAnalysisParser` and was **inert** in consumers,
reporting nothing at all rather than a fault (WR-1462) — measured on three
territories, one of them under a full project run with a warm result cache, since
a warm cache narrows the analysed set to the changed files. If you override this
rule's registration in your own `phpstan.neon`, keep a non-cleaning parser.
...
- **A dedicated decode helper.** emmie ships `App\Support\DateTime\InstantParser`; lokalekeuze's lands under `App\Support\Time`. This is where a request string, a query parameter or a cursor becomes a value object, with the instant-vs-wall-clock question of ADR-0031 answered once and in one place.
- **The row-to-model cast** (`App\Casts`). A database column is genuinely a string arriving from outside, and the cast is the one place the application is entitled to interpret it.
...
Pin to a 0.x minor version today (`^0.10`, the current minor); future 1.0 release will allow `^1.0` pinning. See `CLAUDE.md` § Versioning for the 0.x caret-semantics rationale.
