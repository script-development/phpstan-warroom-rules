...
A union or intersection receiver (`ForeignReader|LeafReader $reader`) is resolved per branch: the call fires when ANY branch's method is a first-party narrowing helper, and the message names the first such branch. A foreign branch does not shadow a first-party one, whichever order the union lists them in.

`?? null` is never flagged — it hands the failure signal to the caller, which is the point. A non-literal fallback (`?? $default`, `?? $this->fetch($leaf)`) is a deliberate miss, as is a helper result laundered through a variable before the coalesce, and a static call through a variable class (`$reader::text($leaf)`).

A sentinel is a **fixed, plausible value written at the call site**: a scalar literal, `true` / `false`, a class constant, an empty array — and a **bare global constant** (`?? SOME_APP_DEFAULT`), which is the same decision with a name on it. There is no allowlist of constant names; a fallback that is genuinely the caller's own default belongs in a variable (`?? $default`), which is a deliberate miss.
