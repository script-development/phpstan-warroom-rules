...
| `ForbidResourceWrappedInJsonResponseRule` | `forbidResourceWrappedInJsonResponse.resourceWrapped` | `response()->json(...)` (the helper `FuncCall` receiver, matched by AST shape) and `new Illuminate\Http\JsonResponse(...)` inside a class whose namespace starts with a `controllerNamespacePrefixes` entry (default `App\Http\Controllers`), when the FIRST argument's resolved type is a subtype of `Illuminate\Http\Resources\Json\JsonResource` | Wrapping a `JsonResource` in an explicit JSON response is an error — a resource is already `Responsable`, so the wrap double-wraps the payload and discards the resource's own response shaping; return the resource directly (`return XxxResource::fromModel($model);`). **Type-aware, deliberately:** a plain array / DTO / scalar / message envelope and `response()->json(null, 204)` are silent, and a resource nested under a named key (`response()->json(['registrations' => Resource::collect(...)])`) is a deliberate envelope whose first argument is an `array`, so it stays silent by design. Doctrine: war-room §Explicit over implicit (#1) + ADR-0009 (resources own their own serialization). Shares `controllerNamespacePrefixes` with `ForbidEloquentMutationInControllersRule` and `EnforceCurrentUserAttributeRule`. |
